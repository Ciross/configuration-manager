...
versions match the authoritative version in `pyproject.toml`. For version 0.3.0,
the build must produce:

```text
dist/configuration_manager-0.3.0-py3-none-any.whl
dist/configuration_manager-0.3.0.tar.gz
```
...
The version command must print `0.3.0`. On Windows, use the corresponding
...
final artifacts. For version 0.3.0, the GitHub Release assets are:

```text
configuration_manager-0.3.0-py3-none-any.whl
configuration_manager-0.3.0.tar.gz
SHA256SUMS.txt
```
...
The current suite contains 19 tests covering authentication and metadata, raw
WMI and v1 access, typed Devices and Collections, Device ↔ Collection
relationships, typed Users, and User ↔ Collection relationships. All 19 must
pass.
